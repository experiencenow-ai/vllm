...
if TYPE_CHECKING:
    ...
    VLLM_DS4_DSV4_SPARSE_MLA_PREFILL_BACKEND: str = "unset"
    ...
    VLLM_WORKSPACE_PREALLOC_BYTES: int = 0
...
environment_variables: dict[str, Callable[[], Any]] = {
    ...
    # calls so the engine sees one cohort while the API response still drains
    # per-prompt outputs. Parking the scheduler during admission is kept as an
    # explicit diagnostic option because it can strand PP services if resume
    # does not restart scheduling correctly.
    ...
    "VLLM_DS4_COHORT_PAUSE_DURING_ADMISSION": lambda: (
    ...
    # DS4 DSV4 PP correctness diagnostic: production uses the fused CUDA layer
    # path. The explicit native-debug mode closes HC state after each sublayer
    # and is only for semantic isolation on tiny prompts.
    "VLLM_DS4_DSV4_LAYER_BACKEND": lambda: os.environ.get(
    ...
    "VLLM_DS4_DSV4_SPARSE_MLA_PREFILL_BACKEND": lambda: os.environ.get(
        "VLLM_DS4_DSV4_SPARSE_MLA_PREFILL_BACKEND", "unset"
    )
    .strip()
    .lower(),
    ...
    "VLLM_DS4_PP_TORCH_PAIR_GROUPS": lambda: (
        os.environ.get("VLLM_DS4_PP_TORCH_PAIR_GROUPS", "0")
        .strip()
        .lower()
        in ("1", "true", "yes", "on")
    ),
    ...
    # Minimum bytes to reserve for each v1 workspace slot before locking.
    "VLLM_WORKSPACE_PREALLOC_BYTES": lambda: int(
        os.getenv("VLLM_WORKSPACE_PREALLOC_BYTES", "0")
    ),
