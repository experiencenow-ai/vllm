...
case "$DS4_PP_TRANSPORT" in
  ...
  torch-pair|torch_pair)
    ...
    ;;
  *)
    echo "DSV4 PP8 unsupported DS4_PP_TRANSPORT=$DS4_PP_TRANSPORT; expected cpu-staged, pynccl-pair, or torch-pair" >&2
    exit 64
    ;;
esac
...
if [[ "$DS4_PP_TRANSPORT" != "cpu-staged" && ! "$VLLM_DS4_PP_DEVICE_TENSOR_DICT_METADATA" =~ ^(1|true|TRUE|yes|YES|on|ON)$ ]]; then
