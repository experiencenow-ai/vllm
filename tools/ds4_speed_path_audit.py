...
check("PP8 exports a stable PP socket ifname list for adjacent NCCL pair groups", "VLLM_DS4_PP_SOCKET_IFNAME" in pp8 and "DS4_PP_SOCKET_IFNAMES" in pp8 and "pp_socket_if=" in pp8)
...
check("parallel_state uses stable PP NCCL ifname list instead of per-edge env mutation", "_ds4_pp_socket_ifnames" in ps and "VLLM_DS4_PP_SOCKET_IFNAME" in ps and "os.environ[\"NCCL_SOCKET_IFNAME\"] = pair_ifname" not in ps)
...
check("P2P preflight uses stable torch pair ifnames and P2P warmup", "_stable_torch_pair_ifnames" in preflight and "torch stable ifnames" in preflight and "batch_isend_irecv(ops)" in preflight and "torch pair warmup failed" in preflight and "dist.all_reduce(value, op=dist.ReduceOp.SUM, group=pair_group)" not in preflight)
