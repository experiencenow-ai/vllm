...
check("PP8 defaults to measured torch ProcessGroup CUDA tensor-dict path", 'VLLM_DS4_PP_TORCH_PG_TENSOR_DICT:-1' in pp8 and 'VLLM_DS4_PP_PYNCCL_TENSOR_DICT:-0' in pp8)
check("PP8 keeps PyNCCL tensor-dict chunking configured for opt-in tests", 'VLLM_DS4_PP_PYNCCL_TENSOR_DICT_STRIPES:-8' in pp8)
check("PP8 enables symmetric PyNCCL P2P credit by default", 'VLLM_DS4_PP_PYNCCL_P2P_CREDIT:-1' in pp8)
check("PP8 keeps PyNCCL warmup enabled by default", 'VLLM_DS4_SKIP_PYNCCL_WARMUP_ALLREDUCE:-0' in pp8)
...
check("parallel_state can send DS4 PP CUDA metadata over torch ProcessGroup", "VLLM_DS4_PP_TORCH_PG_TENSOR_DICT" in ps and "dst=self.ranks[dst], group=self.device_group" in ps and "src=self.ranks[src], group=self.device_group" in ps)
...
check("DSV4 PP8 defaults NCCL preflight to adjacent P2P without world all-reduce", "DS4_NCCL_PREFLIGHT_MODE:-p2p_nccl" in pp8 and "DS4_NCCL_PREFLIGHT_P2P_SKIP_WORLD_ALLREDUCE:-1" in pp8 and "DS4_NCCL_PREFLIGHT_P2P_PAIRS" in pp8)
...
check("PP4xTP2 launcher defaults to measured torch ProcessGroup CUDA tensor-dict path", 'VLLM_DS4_PP_TORCH_PG_TENSOR_DICT:-1' in pp4 and 'VLLM_DS4_PP_PYNCCL_TENSOR_DICT:-0' in pp4)
