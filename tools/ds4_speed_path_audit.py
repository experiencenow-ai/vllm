...
check("TP process-group all-reduce env exists", "VLLM_DS4_TP_PROCESS_GROUP_ALL_REDUCE" in envs)
...
check("parallel_state routes DS4 TP all-reduce through process-group NCCL", "VLLM_DS4_TP_PROCESS_GROUP_ALL_REDUCE" in ps and 'self.unique_name.startswith("tp:")' in ps and "torch.distributed.all_reduce(output, group=self.device_group)" in ps)
...
check("PP4xTP2 launcher uses process-group NCCL for TP all-reduce", 'VLLM_DS4_TP_PROCESS_GROUP_ALL_REDUCE:-1' in pp4 and "tp_pg_all_reduce=" in pp4)
