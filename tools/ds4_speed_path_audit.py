...
check("PP8 defaults to broad NCCL PP tensor-dict path", 'VLLM_DS4_PP_TORCH_PG_TENSOR_DICT:-1' in pp8 and 'VLLM_DS4_PP_TORCH_PAIR_GROUPS:-0' in pp8 and 'VLLM_DS4_PP_TORCH_GROUP_WARMUP:-0' in pp8 and 'VLLM_DS4_PP_PYNCCL_TENSOR_DICT:-0' in pp8)
...
check("PP8 keeps PyNCCL P2P credit available for explicit opt-in tests", 'VLLM_DS4_PP_PYNCCL_P2P_CREDIT:-1' in pp8)
check("PP8 skips PyNCCL warmup by default because live PyNCCL P2P hangs", 'VLLM_DS4_SKIP_PYNCCL_WARMUP_ALLREDUCE:-1' in pp8)
...
check("parallel_state adds reverse credit for unidirectional PyNCCL P2P", "VLLM_DS4_PP_PYNCCL_P2P_CREDIT" in ps and "credit_op" in ps)
...
check("DSV4 PP8 defaults NCCL preflight to broad torch P2P without world all-reduce", "DS4_NCCL_PREFLIGHT_MODE:-p2p_nccl" in pp8 and "DS4_NCCL_PREFLIGHT_P2P_METHOD:-torch_world" in pp8 and "DS4_NCCL_PREFLIGHT_P2P_SKIP_WORLD_ALLREDUCE:-1" in pp8 and "DS4_NCCL_PREFLIGHT_P2P_PAIRS" in pp8)
