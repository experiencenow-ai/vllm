...
check("PP8 defaults to measured torch NCCL pair-group tensor-dict path", 'VLLM_DS4_PP_TORCH_PG_TENSOR_DICT:-1' in pp8 and 'VLLM_DS4_PP_TORCH_PAIR_GROUPS:-1' in pp8 and 'VLLM_DS4_PP_PYNCCL_TENSOR_DICT:-0' in pp8)
...
check("parallel_state builds route-local adjacent torch NCCL pair groups", "_build_ds4_pp_torch_pair_groups" in ps and "VLLM_DS4_PP_NEXT_SOCKET_IFNAME" in ps and "VLLM_DS4_PP_PREV_SOCKET_IFNAME" in ps and "_warm_ds4_pp_torch_pair_group" in ps and "use_local_synchronization=True" in ps)
...
check("P2P preflight creates local-synchronized torch NCCL edge groups", "_new_nccl_pair_group" in preflight and "use_local_synchronization=True" in preflight and "return None" in preflight and "_pp_edge_dev_for_rank(rank, peer)" in preflight)
...
check("DSV4 PP8 defaults NCCL preflight to adjacent torch P2P without world all-reduce", "DS4_NCCL_PREFLIGHT_MODE:-p2p_nccl" in pp8 and "DS4_NCCL_PREFLIGHT_P2P_METHOD:-torch" in pp8 and "DS4_NCCL_PREFLIGHT_P2P_SKIP_WORLD_ALLREDUCE:-1" in pp8 and "DS4_NCCL_PREFLIGHT_P2P_PAIRS" in pp8)
