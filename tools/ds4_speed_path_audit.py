...
check("PP8 exports PP socket hints for adjacent NCCL pair diagnostics", "VLLM_DS4_PP_SOCKET_IFNAME" in pp8 and "DS4_PP_SOCKET_IFNAMES" in pp8 and "pp_socket_if=" in pp8)
...
check("P2P preflight creates local-synchronized torch NCCL edge groups", "_new_nccl_pair_group" in preflight and "use_local_synchronization=True" in preflight and "return None" in preflight and "_route_dev_for_rank(peer)" in preflight)
check("P2P preflight uses pair-local torch ifnames and P2P warmup", "pair_nccl_ifname" in preflight and "os.environ[\"NCCL_SOCKET_IFNAME\"] = route_ifname" in preflight and "_stable_torch_pair_ifnames" not in preflight and "batch_isend_irecv(ops)" in preflight and "torch pair warmup failed" in preflight and "dist.all_reduce(value, op=dist.ReduceOp.SUM, group=pair_group)" not in preflight)
